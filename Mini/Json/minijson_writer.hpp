...
public:
    ...
    {
        write_helper(m_array ? NULL : field_name, value, default_value_writer<V>());
    }

    template<typename V, typename ValueWriter>
    void write(const char* field_name, const V& value, const ValueWriter& value_writer)
    {
        write_helper(m_array ? NULL : field_name, value, value_writer);
    }

    template<typename InputIt>
    void write_array(const char* field_name, InputIt begin, InputIt end)
    {
        write_array(m_array ? NULL : field_name, begin, end, default_value_writer<typename detail::get_value_type<InputIt>::type>());
    }

    template<typename InputIt, typename ValueWriter>
    void write_array(const char* field_name, InputIt begin, InputIt end, ValueWriter value_writer)
    {
        write(m_array ? NULL : field_name, detail::make_range(begin, end), detail::range_writer<InputIt, ValueWriter>(value_writer));
    }

    object_writer nested_object(const char* field_name);

    object_writer nested_array(const char* field_name);
};
...
inline object_writer object_writer::nested_array(const char* field_name)
{
    detail::adjust_stream_settings(stream());
    ...
    auto obj = object_writer(stream(), configuration().increase_nesting_level());
    obj.m_array = true;
    return obj;
}
