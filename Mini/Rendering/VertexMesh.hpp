...
public:
    ...
    void AddQuad( Vector2 center, Vector2 size, Color color )
    {
    ...
    void AddCube(Vector3 center, Vector3 size) {
        AddCube(center, size, Color::White());
    }

    void AddCube(Vector3 center, Vector3 size, Color color) {
    ...
    void AddPlane(const Mini::Vector3 &center, const Mini::Vector2 &size, const Mini::Box &textureCoords) {
        AddPlane(center, size, textureCoords, Color::White());
    }

    void AddPlane(const Mini::Vector3 &center, const Mini::Vector2 &size, const Mini::Box &textureCoords, const Mini::Color &color) {
    ...
    void AddTriangle(
        ...
        const Color& color, const Matrix4x4& world) {
        GLshort index = vertices.size();
    ...
    void AddSegmentedQuad(const Mini::Matrix4x4 &world, const Mini::Vector2 &size, const Mini::Vector2 &blTexCoord, const Mini::Vector2 &trTexCoord, int segments) {
    ...
    void AddGeoSphere(const Mini::Vector3 center, float radius, int segments) {
    ...
    void AddSphere(const Mini::Vector3 center, float radius, int xSegments, int ySegments) {
        ...
        for (int y = 0; y<=ySegments; y++) {
            for(int x = 0; x<=xSegments; x++) {
                ...
                vertices.push_back({center + position, Vector2(x/(float)xSegments, y/(float)ySegments), Color::White(), position.Normalized()});
            }
        }
